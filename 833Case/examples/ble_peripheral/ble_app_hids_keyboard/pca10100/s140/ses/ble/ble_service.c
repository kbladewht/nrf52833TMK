...
APP_TIMER_DEF(m_battery_timer_id2);                                  /**< Battery timer. */
...
static void battery_level_meas_timeout_handler2(void * p_context)
{
    NRF_LOG_INFO("HID Keyboard example startedsdfs.");
}
 ...
 void timers_init(void)
{
    ...
    err_code = app_timer_create(&m_battery_timer_id,
        ...
        // Create battery timer.
    err_code = app_timer_create(&m_battery_timer_id2,
                                APP_TIMER_MODE_REPEATED,
                                battery_level_meas_timeout_handler2);
    APP_ERROR_CHECK(err_code);
}
                                      ...
                                      uint16_t   * p_actual_len)
{
    ...
    if (m_in_boot_mode) {
        ...
        err_code = ble_hids_inp_rep_send(p_hids,
            ...
            p_key_pattern,
            m_conn_handle);
// kb_nrf_print("errorCode111 is  %02X", err_code);
...
void init_and_start_scan_timer(void)
{
    ...
    err_code = app_timer_start(m_keyboard_scan_timer_id,  SCAN_INTERVAL, NULL);
    APP_ERROR_CHECK(err_code);
}